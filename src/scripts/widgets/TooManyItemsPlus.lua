...
local function WormwoodI()
    local bs = 1
    local ctrlks = TheInput:IsKeyDown(KEY_CTRL)
    if ctrlks then
        bs = 0
    end
    local fnstr = 'local p = player.components.bloomness if player and not player:HasTag("playerghost") and p then p:SetLevel(' .. bs .. ') end'
    SendCommand(string.format(IsPlayerExist .. fnstr, GetCharacter()))
    OperateAnnnounce(STRINGS.TOO_MANY_ITEMS_UI.BUTTON_WORMWOOD_LV1)
end
local function WormwoodII()
    local bs = 2
    local ctrlks = TheInput:IsKeyDown(KEY_CTRL)
    if ctrlks then
        bs = 0
...
local TooManyItems =
    Class(
        ...
        function(self)
            ...
            self.shieldpos_x = -340 --窗口横向位置
            --默认-30  self.shieldpos_y = -30
            self.shieldpos_y = 0 --窗口纵向位置
            --默认338 self.shieldsize_x = 338
            self.shieldsize_x = 420 --窗口整体宽度
            --默认480 self.shieldsize_y = 480
            self.shieldsize_y = 480 --窗口整体高度
...
function TooManyItems:ShowWormwoodMenu()
    self.abigailshield:Hide()
    self.woodieshield:Hide()
    self.abigailshield.flag = false
    self.woodieshield.flag = false
...
function TooManyItems:AbigailMenu()
    self.abigailshield = self.root:AddChild(Image("images/ui.xml", "black.tex"))
    self.abigailshield:SetScale(1, 1, 1)
    self.abigailshield:SetPosition(
        self.shieldpos_x - self.shieldsize_x * 0.5 - 20,
        self.shieldpos_y - self.shieldsize_y * 0.5 + 60,
        0
    )
    ...
    for k, v in pairs(self.abigailbuttonlist) do
        MakeMainButton(self.abigailshield, self.abigailbuttons, k, v.tip, v.fn, v.atlas, v.image, v.pos)
    end
end

function TooManyItems:WormwoodMenu()
    self.wormwoodshield = self.root:AddChild(Image("images/ui.xml", "black.tex"))
    ...
    for k, v in pairs(self.wormwoodbuttonlist) do
        MakeMainButton(self.wormwoodshield, self.wormwoodbuttons, k, v.tip, v.fn, v.atlas, v.image, v.pos)
    end
end
